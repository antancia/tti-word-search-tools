...
export default function WordSearch() {
  ...
  const [excludeLettersInWords, setExcludeLettersInWords] = useState<boolean>(false)
  ...
  const findWordPositions = (word: string, isBackwards: boolean = false): Position[][] => {
    ...
    if (isBackwards) {
      ...
      for (let startRow = grid.length - 1; startRow >= word.length - 1; startRow--) {
        for (let startCol = grid[0].length - 1; startCol >= word.length - 1; startCol--) {
          let match = true
      ...
      for (let startRow = 0; startRow <= grid.length - word.length; startRow++) {
        for (let startCol = word.length - 1; startCol < grid[0].length; startCol++) {
          let match = true
          const wordPositions: Position[] = []
          for (let i = 0; i < word.length; i++) {
            const row = startRow + i
            const col = startCol - i
            if (row >= grid.length || col < 0 || grid[row][col] !== wordUpper[i]) {
              match = false
              break
            }
            wordPositions.push({ row, col })
      ...
      for (let startRow = 0; startRow <= grid.length - word.length; startRow++) {
        for (let startCol = 0; startCol <= grid[0].length - word.length; startCol++) {
          let match = true
          const wordPositions: Position[] = []
          for (let i = 0; i < word.length; i++) {
            const row = startRow + i
            const col = startCol + i
            if (row >= grid.length || col >= grid[0].length || grid[row][col] !== wordUpper[i]) {
              match = false
              break
            }
            wordPositions.push({ row, col })
      ...
      for (let startRow = grid.length - 1; startRow >= word.length - 1; startRow--) {
        for (let startCol = 0; startCol <= grid[0].length - word.length; startCol++) {
          ...
          for (let i = 0; i < word.length; i++) {
            const row = startRow - i
            const col = startCol + i
            if (row < 0 || col >= grid[0].length || grid[row][col] !== wordUpper[i]) {
              match = false
              break
            }
            wordPositions.push({ row, col })
  ...
  const shareEdge = (instance1: WordInstance, instance2: WordInstance): boolean => {
  ...
  const blendColors = (colors: string[]): string => {
    ...
    const rgbaValues = colors.map(color => {
    ...
    }).filter(v => v !== null) as Array<{ r: number; g: number; b: number; a: number }>
    
    if (rgbaValues.length === 0) return ''
    
    const avg = rgbaValues.reduce((acc, val) => ({
      r: acc.r + val.r,
      g: acc.g + val.g,
      b: acc.b + val.b,
      a: acc.a + val.a
    ...
    return `rgba(${Math.round(avg.r / count)}, ${Math.round(avg.g / count)}, ${Math.round(avg.b / count)}, ${Math.min(0.9, avg.a / count + 0.1)})`
  }
  ...
  const cellHighlights = useMemo(() => {
    ...
    for (let i = 0; i < allWordInstances.length; i++) {
      ...
      let colorId = 0
      while (true) {
        const isColorUsed = Array.from(related).some(j => assignedColors.get(j) === colorId)
        if (!isColorUsed) {
          break
        }
        colorId++
    ...
    return highlights
  }, [highlightForwards, highlightBackwards, forwardsWords, backwardsWords, isCryptogramActive])
          ...
          {grid.map((row, rowIndex) => (
            <div key={rowIndex} className="grid-row">
              {row.split('').map((letter, colIndex) => {
                ...
                const highlight = cellHighlights[key] || { wordInstances: [] }
                ...
                if (encodeCryptogram) {
                  ...
                  displayLetter = cryptogramEncodeMapping[letter.toUpperCase()] || letter
                } else if (decodeCryptogram) {
                  // Decode: replace key letter with original letter
                  displayLetter = cryptogramDecodeMapping[letter.toUpperCase()] || letter
                }
                ...
                if (wordInstances.length > 0) {
                  ...
                  const colors = wordInstances.map(instance => {
                    const colorId = instance.colorId % (instance.isBackwards ? backwardsColors.length : forwardsColors.length)
                    return instance.isBackwards ? backwardsColors[colorId] : forwardsColors[colorId]
                  })
                ...
                const shouldExclude = excludeLettersInWords && isPartOfHighlightedWord
                const isLetterHighlighted = selectedLetters.has(displayLetter.toUpperCase()) && !shouldExclude
                const letterHighlightColor = isLetterHighlighted ? 'rgba(0, 255, 255, 0.6)' : '' // Cyan color to distinguish from word highlights
                ...
                if (isLetterHighlighted) {
                  if (backgroundColor) {
                    ...
                    finalBackgroundColor = blendColors([backgroundColor, letterHighlightColor])
                  } else {
                    finalBackgroundColor = letterHighlightColor
                  }
                }
                
                const className = `grid-cell ${wordInstances.length > 0 ? 'highlighted' : ''} ${isLetterHighlighted ? 'letter-highlighted' : ''}`
                const style = finalBackgroundColor ? { backgroundColor: finalBackgroundColor } : {}
